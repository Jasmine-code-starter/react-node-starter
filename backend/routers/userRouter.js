...
userRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (findUser) {
      return res.errorHanler(
        "Username has existed, Please change the username!"
      );
    }
    ...
    if (findUser) {
      if (bcrypt.compareSync(userInfo.password, findUser.password)) {
        return res.send({
          ...
          token: "Bearer" + generateToken(findUser),
          id: findUser.id,
        });
      } else {
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (user) {
      ...
      res.send({
        ...
        token: "Bearer" + generateToken(updateUser),
        user: updateUser,
      });
    } else {
...
userRouter.post(
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    if (album) {
      res
        .status(200)
        .send({ status: 0, message: "Upload done!", album: album });
    } else {
